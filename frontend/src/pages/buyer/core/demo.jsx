...
// Import các icons
import { 
  FaBoxOpen, FaFire, FaRobot, FaArrowTrendUp, FaStar, 
  FaTicketSimple, FaTags, FaHeadset, FaPhoneVolume 
...
const Home = () => {
  ...
  if (loading) return (
    <div className="min-h-screen flex flex-col justify-center items-center text-[#ee4d2d]">
      ...
      <main className="bg-[#f5f5f5] pb-10 flex-grow">
        <div className="w-full pt-8">
          ...
          <div className="flex justify-between gap-0 w-full relative">
                ...
                <div className="bg-white rounded-sm shadow-sm p-3 max-h-[calc(100vh-140px)] overflow-y-auto custom-scrollbar">
                    ...
                    <div className="flex flex-col gap-3">
                        ...
                        >
                          ...
                          <span className="text-[12px] text-gray-700 font-medium group-hover:text-[#ee4d2d]">Tất cả</span>
                        </div>
                        {categories.map((catName, index) => {
                            ...
                            return (
                                <div 
                                  ...
                                  className="flex items-center gap-2 cursor-pointer group relative hover:bg-gray-50 p-1 rounded transition-colors"
                                >
                                    {trend === "TĂNG" && <FaArrowTrendUp className="absolute top-1 right-1 text-red-500 text-[8px] animate-bounce" />}
                                    {trend === "NEW" && <span className="absolute top-1 right-1 bg-green-500 text-white text-[6px] px-1 rounded">NEW</span>}
                ...
                <div className="w-full max-w-[1200px]">
                    ...
                    <div className="bg-white p-5 rounded-sm shadow-sm mb-6 border-t-2 border-[#ee4d2d]">
                        <div className="flex justify-between items-center mb-4 border-b pb-2">
                           <div className="text-base font-bold text-[#ee4d2d] uppercase italic flex items-center gap-2">
                                   ...
                                   <div key={item.id} className="px-2 cursor-pointer group" onClick={() => handleProductClick(item.id)}>
                                      <div className="relative rounded-sm overflow-hidden bg-gray-50 aspect-square">
                                         <div className="absolute top-0 left-0 z-20">
                                            <div className="bg-[#ee4d2d] text-white text-[10px] font-bold px-1.5 py-0.5 rounded-br-sm">HOT</div>
                                         </div>
                                         <img 
                                      ...
                                      <div className="mt-2 text-sm text-gray-700 font-medium line-clamp-2 text-center h-10 group-hover:text-[#ee4d2d]">
                                         {item.name}
                                      </div>
                                   </div>
                                ))}
                             ...
                             <Slider {...sliderSettings.topProduct}>
                                {globalTrending.map((item) => (
                                   <div key={item.id} className="px-2 cursor-pointer group" onClick={() => handleProductClick(item.id)}>
                                      ...
                                      <div className="text-center text-[#ee4d2d] font-bold text-sm">
                    ...
                    <div ref={suggestedRef} className="sticky top-[135px] z-20 bg-[#f5f5f5] mb-2">
                        <div className="py-3 text-[#ee4d2d] font-bold bg-white border-b-4 border-[#ee4d2d] cursor-pointer uppercase flex items-center justify-center gap-2 shadow-sm rounded-sm">
                        ...
                        {suggested && suggested.length > 0 ? (
                          suggested.map((item) => (
                            <div 
                              ...
                              className="bg-white hover:border-[#ee4d2d] border border-transparent rounded-sm shadow-sm cursor-pointer overflow-hidden flex flex-col relative group transition-all hover:-translate-y-1 hover:shadow-lg"
                            >
                              <div className="absolute top-0 right-0 z-10">
                                  <div className="bg-[#feeeea] text-[#ee4d2d] text-[9px] px-1 border border-[#ee4d2d] rounded-bl-sm font-bold flex items-center gap-1">
                                     <FaRobot size={8}/> AI
                                  </div>
                              </div>
                              ...
                              <div className="p-2 flex flex-col justify-between flex-1">
                                  <div className="text-[12px] text-gray-800 line-clamp-2 mb-2 h-8 leading-4 group-hover:text-[#ee4d2d]">{item.name}</div>
                    ...
                    <div className="mt-10 text-center pb-10">
                        <button onClick={fetchAllProducts} className="bg-white border border-[#ee4d2d] text-[#ee4d2d] px-12 py-2.5 hover:bg-[#feeeea] rounded-sm shadow-sm transition-all text-sm font-bold uppercase tracking-wide">
                          Xem thêm
                        </button>
                    </div>
                </div>
            ...
            {/* 3. SIDEBAR PHẢI: MÃ GIẢM GIÁ & TIỆN ÍCH */}
            <aside className="w-[180px] flex-shrink-0 sticky top-[170px] h-fit pr-4 hidden xl:block z-30">
                <div className="flex flex-col gap-4">
                    ...
                    <div className="bg-white rounded-sm shadow-sm p-3">
                        <div className="flex items-center gap-2 text-[#ee4d2d] font-bold text-xs uppercase mb-3 border-b border-gray-100 pb-2">
                           <FaTicketSimple /> Mã giảm giá
                        </div>
                        <div className="flex flex-col gap-2">
                           {/* Voucher Item */}
                           <div className="border border-[#ee4d2d] bg-[#feeeea] p-2 rounded relative overflow-hidden group cursor-pointer">
                              <div className="text-[10px] text-[#ee4d2d] font-bold">GIẢM 50%</div>
                              <div className="text-[9px] text-gray-600">Đơn từ 0đ</div>
                              <div className="mt-1 bg-[#ee4d2d] text-white text-[9px] text-center rounded py-0.5 hover:bg-red-600 transition-colors">Lưu</div>
                    ...
                    {/* WIDGET 2: Từ Khóa Hot */}
                    <div className="bg-white rounded-sm shadow-sm p-3">
                        <div className="flex items-center gap-2 text-gray-700 font-bold text-xs uppercase mb-3 border-b border-gray-100 pb-2">
                           <FaTags /> Từ khóa Hot
                        </div>
                        <div className="flex flex-wrap gap-1">
                           {['Áo thun', 'Giày', 'Iphone 15', 'Son môi', 'Sách'].map((tag, i) => (
                              <span key={i} className="text-[10px] bg-gray-100 hover:bg-gray-200 text-gray-600 px-2 py-1 rounded cursor-pointer transition-colors">
                                 #{tag}
                              </span>
                           ))}
                        </div>
